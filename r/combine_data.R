...
data_files <- list.files("results/", pattern = "*.csv") %>% as.data.frame()

names(data_files) <- c("url")

data_files <- data_files %>% 
  mutate(change_time = file.mtime(paste0("results/",url)))



if (file.exists("data/comb_data.RDS") != TRUE){
  ...
  saveRDS(result,"data/comb_data.RDS")
} else {
